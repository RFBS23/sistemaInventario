...
namespace presentacion
{
    public partial class frmVentasTienda : Form
    {
        ...
        {
            Productos oProductos = new N_Ptienda().Listar().Where(p => p.codigo == txtcodigoprod.Text).FirstOrDefault();
            if (oProductos != null)
            {
        ...
        private void btnagregar_Click(object sender, EventArgs e)
        {
            ...
            if (!producto_existe)
            {
                bool respuesta = new N_Ventas().RestarStock(Convert.ToInt32(txtidproducto.Text), Convert.ToInt32(txtcantidadprod.Value.ToString()));
                if (respuesta)
                {
        ...
        private void btnbuscarprove_Click(object sender, EventArgs e)
        {
            using (var modal = new mdProdTienda())
            {
                ...
                if (result == DialogResult.OK)
                {
                    txtidproducto.Text = modal._ProductosTienda.idproducto.ToString();
                    txtcodigoprod.Text = modal._ProductosTienda.codigo;
                    txtnombre.Text = modal._ProductosTienda.nombre;
                    txtstock.Text = modal._ProductosTienda.stock.ToString();
                    txtprecioventa.Text = modal._ProductosTienda.precioventa.ToString("0.00");
                    listatallas.Text = modal._ProductosTienda.oTallasropa.nombretalla.ToString();
                    txtcolores.Text = modal._ProductosTienda.colores.ToString();
